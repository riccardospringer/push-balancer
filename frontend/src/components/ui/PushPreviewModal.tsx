...
function now(): { time: string; date: string; dateShort: string } {
  ...
  const dateShort = d.toLocaleDateString('de-DE', { weekday: 'short', day: 'numeric', month: 'long' })
  return { time, date, dateShort }
}

// ── iOS Phone Preview ──────────────────────────────────────────────────────

function IOSPreview({ title, dachzeile, time, date }: { title: string; dachzeile: string; time: string; date: string }) {
  return (
    <div style={{ textAlign: 'center' }}>
      ...
      >
        ...
        >
          <span style={{ color: 'rgba(255,255,255,0.9)', fontSize: '11px', fontWeight: 600 }}>{time}</span>
        ...
        {/* Clock */}
        <div style={{ color: 'white', fontSize: '44px', fontWeight: 300, lineHeight: 1, marginBottom: '2px', letterSpacing: '-1px' }}>
          {time}
        </div>
        <div style={{ color: 'rgba(255,255,255,0.7)', fontSize: '13px', marginBottom: '16px' }}>
          {date}
        </div>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
            ...
            <span style={{ color: 'rgba(255,255,255,0.8)', fontSize: '10px', fontWeight: 600, flex: 1 }}>BILD</span>
            <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: '10px' }}>Jetzt</span>
          </div>
          {dachzeile && (
            <div
              style={{
...
function AndroidPreview({ title, dachzeile, time, dateShort }: { title: string; dachzeile: string; time: string; dateShort: string }) {
  return (
    <div style={{ textAlign: 'center' }}>
      <div style={{ fontSize: '11px', color: 'var(--text-secondary)', marginBottom: '8px', fontWeight: 500 }}>
        Android (BILD App)
      ...
      >
        ...
        >
          <span style={{ color: 'rgba(255,255,255,0.8)', fontSize: '11px' }}>{time}</span>
          <div style={{ display: 'flex', gap: '4px' }}>
            <svg width="12" height="10" viewBox="0 0 24 24" fill="rgba(255,255,255,0.7)">
              <path d="M1 9l2 2c4.97-4.97 13.03-4.97 18 0l2-2C16.93 2.93 7.08 2.93 1 9zm8 8l3 3 3-3c-1.65-1.66-4.34-1.66-6 0zm-4-4l2 2c2.76-2.76 7.24-2.76 10 0l2-2C15.14 9.14 8.87 9.14 5 13z" />
            </svg>
            <svg width="12" height="10" viewBox="0 0 24 24" fill="rgba(255,255,255,0.7)">
              <rect x="17" y="4" width="4" height="16" rx="1" />
        ...
        <div style={{ marginBottom: '20px', textAlign: 'center' }}>
          <div style={{ color: 'white', fontSize: '52px', fontWeight: 200, lineHeight: 1, letterSpacing: '-2px' }}>
            {time}
          </div>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
            ...
            <span style={{ color: 'rgba(255,255,255,0.6)', fontSize: '10px', flex: 1 }}>BILD</span>
            <div
              style={{
                width: '6px',
                height: '6px',
        ...
        >
          {['‹', '○', '□'].map((s, i) => (
            <span key={i} style={{ color: 'rgba(255,255,255,0.4)', fontSize: '18px', lineHeight: 1 }}>{s}</span>
          ))}
        </div>
      </div>
    </div>
...
function CharCount({ value, max }: { value: string; max: number }) {
  ...
  const color = len > max ? 'var(--red)' : len > max * 0.85 ? 'var(--amber)' : 'var(--text-tertiary)'
  return (
    <div style={{ fontSize: '11px', color, textAlign: 'right', marginTop: '3px' }}>
      {len} / {max} Zeichen
    </div>
  )
}
...
export function PushPreviewModal({ article, onClose }: PushPreviewModalProps) {
  ...
  const handleGenerateTitle = useCallback(() => {
    mutate({ url: article.url, title: article.title, category: article.category })
  }, [mutate, article])
  ...
  return (
    <Modal open onClose={onClose} title="Push-Vorschau" width={700}>
      ...
      >
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: '12px', fontWeight: 500, lineHeight: 1.4, marginBottom: '4px' }}>
            {article.title}
          </div>
      ...
      <div style={{ marginBottom: '16px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
        ...
        >
          {isPending ? <><Spinner size={13} color="#fff" /> Generiere…</> : 'KI-Titel generieren'}
        </Button>

        {error && (
          <div style={{ fontSize: '12px', color: 'var(--red)', padding: '6px 10px', background: 'var(--red-bg)', borderRadius: '5px' }}>
            KI-Titel Generierung fehlgeschlagen
          </div>
        )}

        {/* Alternative titles */}
        {aiResult?.alternativeTitles && aiResult.alternativeTitles.length > 0 && (
          <div>
            <div style={{ fontSize: '12px', color: 'var(--text-secondary)', marginBottom: '5px', fontWeight: 500 }}>Alternativen:</div>
      ...
      >
        <IOSPreview title={title} dachzeile={dachzeile} time={time} date={date} />
        <AndroidPreview title={title} dachzeile={dachzeile} time={time} dateShort={dateShort} />
      </div>
