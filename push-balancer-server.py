...
def _generate_server_feedback(state):
    ...
    for push in mature:
        ...
        # Fallback: ts_num + title-hash (fuer Pushes ohne message_id)
        _msg_id = push.get('message_id', '')
        if _msg_id:
            push_key = _msg_id
        else:
            import hashlib
    ...
    while True:
        try:
            ...
            try:
                ...
                _coverage_stats["last_error"] = f"{type(_cov_e).__name__}: {_cov_e}"
                log.warning(f"[Coverage] Worker-Fehler: {_cov_e}")
            # LLM Relevanz-Gremium fuer Gap-Artikel (rate-limited, max alle 5 Min)
            try:
                _llm_relevance_check()
            except Exception as _rel_e:
                _relevance_stats["last_error"] = f"{type(_rel_e).__name__}: {_rel_e}"
                log.warning(f"[Relevanz] Worker-Fehler: {_rel_e}")
        except Exception as e:
...
def _gather_bild_context():
    """Sammelt BILD-Push-Titel (24h) und Sitemap-Titel als Kontext fuer Coverage-Check."""
    push_titles = []
    try:
        with _push_db_lock:
            _conn = sqlite3.connect(PUSH_DB_PATH, timeout=5)
            cutoff = int(time.time()) - 86400
            rows = _conn.execute(
                "SELECT title FROM pushes WHERE ts_num >= ? AND title IS NOT NULL AND title <> '' ORDER BY ts_num DESC LIMIT 200",
                (cutoff,)
            ).fetchall()
            _conn.close()
            push_titles = [r[0] for r in rows if r[0]]
    except Exception as e:
        log.warning(f"[Coverage] BILD-Push-Kontext Fehler: {e}")

    sitemap_titles = []
    try:
        xml = _fetch_url("https://www.bild.de/sitemap-news.xml")
        if xml:
            items = _parse_rss_items_standalone(xml, max_items=120)
            sitemap_titles = [it.get("t", "") for it in items if it.get("t")]
    except Exception as e:
        log.warning(f"[Coverage] BILD-Sitemap-Kontext Fehler: {e}")

    return push_titles, sitemap_titles


def _llm_coverage_step1(article_titles, bild_pushes, bild_sitemap):
    """Step 1: GPT-4o-mini Event-Extraktion + Pre-Match. Batch bis 20 Titel."""
    global _coverage_stats
    api_key = os.environ.get("OPENAI_API_KEY", "")
    if not api_key or not article_titles:
        return []

    # BILD-Kontext formatieren (gekuerzt fuer Token-Budget)
    push_block = "\n".join(f"- {t[:80]}" for t in bild_pushes[:80])
    sitemap_block = "\n".join(f"- {t[:80]}" for t in bild_sitemap[:60])

    articles_block = "\n".join(f"{i}. {t[:100]}" for i, t in enumerate(article_titles))

    prompt = f"""Du bist ein Nachrichtenredakteur bei BILD. Pruefe fuer jeden Konkurrenz-Artikel, ob BILD dieses exakte Nachrichtenereignis bereits abdeckt.

WICHTIG:
- Gleiche Personen oder Themen bedeuten NICHT automatisch gleiche Berichterstattung.
- "Trump Zoelle China" und "Trump Golfplatz" sind VERSCHIEDENE Stories.
- Nur wenn das KERNEREREIGNIS (wer + was + wann) uebereinstimmt = abgedeckt.
- Im Zweifel: NICHT abgedeckt (lieber Gap melden als uebersehen).

BILD Push-Titel (letzte 24h):
{push_block}

BILD Sitemap-Titel (aktuell):
{sitemap_block}

Konkurrenz-Artikel zu pruefen:
{articles_block}

Antworte NUR mit einem JSON-Array. Fuer JEDEN Artikel:
[
  {{"idx": 0, "covered": true/false, "confidence": 0.0-1.0, "matched_bild_title": "Titel oder null", "reason": "1 Satz"}}
]

Confidence: >0.8 = sicher, 0.2-0.8 = unklar, <0.2 = sicher nicht abgedeckt."""
    ...
    try:
        ...
        _coverage_stats["last_error"] = f"Step1: {e}"
        log.warning(f"[Coverage] Step1 Fehler: {e}")
    return []


def _llm_coverage_step2(ambiguous_items, bild_pushes, bild_sitemap):
    """Step 2: GPT-4o definitives Urteil fuer ambiguous Cases. Max 5 Calls, 3 Artikel/Call."""
    global _coverage_stats
    api_key = os.environ.get("OPENAI_API_KEY", "")
    ...
    results = []
    # Batch: 3 Artikel pro Call, max 5 Calls
    batches = [ambiguous_items[i:i+3] for i in range(0, min(len(ambiguous_items), 15), 3)]

    for batch in batches[:5]:
        articles_text = ""
        for item in batch:
            matches_text = ""
            if item.get("matched_bild_title"):
                matches_text = f"\n  Moeglicher BILD-Match: \"{item['matched_bild_title']}\""
            articles_text += f"\n- Konkurrenz: \"{item['title']}\"{matches_text}\n  Voranalyse-Grund: {item.get('reason', 'unklar')}\n"

        prompt = f"""Du bist Senior-Redakteur bei BILD. Entscheide definitiv fuer jeden Artikel: Deckt BILD dieses Nachrichtenereignis ab?

Regeln:
- Gleiches KERNEREREIGNIS (Person + Handlung + Zeitpunkt) = abgedeckt
- Gleiche Person, ANDERES Ereignis = NICHT abgedeckt
- Gleiches Thema, anderer Aspekt/Zeitpunkt = NICHT abgedeckt
- Im Zweifel: NICHT abgedeckt

Artikel:
{articles_text}

Antworte NUR mit JSON-Array:
[{{"title": "Originaltitel", "covered": true/false, "confidence": 0.0-1.0, "matched_title": "BILD-Titel oder null", "reason": "1 Satz"}}]"""
        ...
        try:
            ...
            _coverage_stats["last_error"] = f"Step2: {e}"
            log.warning(f"[Coverage] Step2 Fehler: {e}")

    return results


def _llm_coverage_check():
    """Orchestrator: Sammelt alle Feed-Artikel, prueft Cache, ruft Step1+Step2 auf."""
    global _coverage_last_cycle_ts, _coverage_stats

    now = time.time()
    if now - _coverage_last_cycle_ts < _COVERAGE_CYCLE_INTERVAL:
        return  # Rate-Limit
    _coverage_last_cycle_ts = now

    api_key = os.environ.get("OPENAI_API_KEY", "")
    if not api_key:
        return

    # Alle Artikel aus Feed-Cache sammeln (priorisiert: competitors > intl > sport)
    _MAX_PER_CYCLE = 120  # max 6 Batches a 20 = ~90s
    uncached = []  # [(title, link, cache_key)]
    cached_hits = 0

    for feed_type in ("competitors", "international", "sport_competitors", "sport_europa", "sport_global"):
        data = _feed_cache.get(feed_type, {}).get("data", {})
        for outlet, items in data.items():
            if not isinstance(items, list):
                continue
            for item in items:
                title = item.get("t", "")
                ...
                if not title or len(title) < 5:
                    continue
                key = _coverage_cache_key(title, link)
                with _coverage_cache_lock:
                    cached = _coverage_cache.get(key)
                    if cached and (now - cached["ts"]) < _COVERAGE_CACHE_TTL:
                        cached_hits += 1
                        continue
                uncached.append((title, link, key))

    _coverage_stats["cache_hits"] = cached_hits
    total_uncached = len(uncached)
    if not uncached:
        _coverage_stats["debug"] = f"all_cached ({cached_hits})"
        return

    # Cap auf Max pro Zyklus (Rest kommt im naechsten Zyklus via expired cache)
    if len(uncached) > _MAX_PER_CYCLE:
        uncached = uncached[:_MAX_PER_CYCLE]

    t0 = time.time()
    bild_pushes, bild_sitemap = _gather_bild_context()
    if not bild_pushes and not bild_sitemap:
        _coverage_stats["debug"] = "no_bild_context"
        return

    # Step 1: Batch-weise GPT-4o-mini + sofortiges Cache-Schreiben
    titles_only = [t for t, l, k in uncached]
    n_batches = (len(titles_only) + 19) // 20
    ambiguous = []
    batch_cached = 0

    _coverage_stats["debug"] = f"processing {len(uncached)}/{total_uncached} in {n_batches} batches"

    for batch_start in range(0, len(titles_only), 20):
        batch = titles_only[batch_start:batch_start + 20]
        step1 = _llm_coverage_step1(batch, bild_pushes, bild_sitemap)

        # Sofort verarbeiten und in Cache schreiben
        for r in step1:
            try:
                idx = int(r.get("idx", 0)) + batch_start
            except (TypeError, ValueError):
                idx = batch_start
            if not (0 <= idx < len(uncached)):
                continue
            ...
            covered_val = r.get("covered")
            if isinstance(covered_val, str):
                covered_val = covered_val.lower() in ("true", "yes", "1")
            else:
                covered_val = bool(covered_val) if covered_val is not None else False

            if conf > 0.8 or conf < 0.2:
                # Sicher → sofort in Cache
                with _coverage_cache_lock:
                    _coverage_cache[key_val] = {
                        "covered": covered_val,
                        "matched_title": r.get("matched_bild_title") or "",
                        "confidence": conf,
                ...
                ambiguous.append({
                    ...
                    "link": link_val,
                    "key": key_val,
                    "matched_bild_title": r.get("matched_bild_title") or "",
                    "reason": r.get("reason") or "",
                    "step1_covered": covered_val,
                    "step1_confidence": conf,
                })

        _coverage_stats["debug"] = f"batch {batch_start//20+1}/{n_batches}: {batch_cached} cached, {len(ambiguous)} ambig"

    # Step 2: GPT-4o fuer ambiguous
    if ambiguous:
        log.info(f"[Coverage] {len(ambiguous)} ambiguous → Step2")
        try:
            step2_results = _llm_coverage_step2(ambiguous, bild_pushes, bild_sitemap)
        except Exception as e:
            log.warning(f"[Coverage] Step2 Fehler: {e}")
            step2_results = []
        # Step 2 Ergebnisse matchen
        step2_by_title = {}
        for r in step2_results:
            t = (r.get("title") or "").strip().lower()[:120]
            if t:
                step2_by_title[t] = r
        ...
        for item in ambiguous:
            ...
            if s2:
                try:
                    s2_conf = float(s2.get("confidence") or 0.7)
                except (TypeError, ValueError):
                    s2_conf = 0.7
                s2_covered = s2.get("covered")
                if isinstance(s2_covered, str):
                    s2_covered = s2_covered.lower() in ("true", "yes", "1")
                else:
                    s2_covered = bool(s2_covered) if s2_covered is not None else False
                with _coverage_cache_lock:
                    _coverage_cache[item["key"]] = {
                        "covered": s2_covered,
                        "matched_title": s2.get("matched_title") or "",
                        "confidence": s2_conf,
                        "reason": s2.get("reason") or "",
                        "ts": now,
                        "source": "step2",
                        "title_key": t_key,
                    }
    ...
    with _coverage_cache_lock:
        expired = [k for k, v in _coverage_cache.items() if now - v["ts"] > 3600]
        for k in expired:
            del _coverage_cache[k]

    elapsed = int((time.time() - t0) * 1000)
    _coverage_stats["last_cycle_ms"] = elapsed
    n_step2 = len(ambiguous)
    with _coverage_cache_lock:
        final_size = len(_coverage_cache)
    _coverage_stats["debug"] = f"done: {batch_cached}+{n_step2} in {elapsed}ms, cache={final_size}, total_feeds={total_uncached}"
    log.info(f"[Coverage] Zyklus: {len(uncached)} verarbeitet ({total_uncached} total), {cached_hits} cached, {n_step2}→step2, {batch_cached} direkt, {elapsed}ms")


# ═══════════════════════════════════════════════════════════════════════════════
#  LLM-AGENTEN-GREMIUM: BILD-Relevanz-Bewertung fuer Gap-Artikel
#  3 Agenten: Nachrichtenwert-Analyst → BILD-DNA-Experte → Chefredakteur-Jury
# ═══════════════════════════════════════════════════════════════════════════════

_relevance_cache = {}  # md5(title) → {relevant, score, reason, agent1, agent2, agent3, ts}
_relevance_cache_lock = threading.Lock()
_RELEVANCE_CACHE_TTL = 900  # 15 Minuten
_RELEVANCE_CYCLE_INTERVAL = 300  # 5 Minuten
_relevance_last_cycle_ts = 0
_relevance_stats = {
    "agent1_calls": 0, "agent2_calls": 0, "agent3_calls": 0,
    "cache_hits": 0, "last_cycle_ms": 0, "last_error": "", "debug": "",
}


def _relevance_cache_key(title):
    raw = title.strip().lower()[:120].encode("utf-8")
    return _hashlib.md5(raw).hexdigest()


def _relevance_agent1_nachrichtenwert(titles):
    """Agent 1: Nachrichtenwert-Analyst — Massentauglichkeit, Prominenz, Dramatik."""
    global _relevance_stats
    api_key = os.environ.get("OPENAI_API_KEY", "")
    if not api_key or not titles:
        return []

    articles_block = "\n".join(f"{i}. {t[:100]}" for i, t in enumerate(titles))

    prompt = f"""Du bist Nachrichtenwert-Analyst. Bewerte jeden Artikel: Wie hoch ist der Nachrichtenwert fuer ein Massenpublikum in Deutschland?

BEWERTUNGSKRITERIEN:
- Prominenz: Bekannte Persoenlichkeiten, grosse Vereine, Staatschefs (1-10)
- Dramatik: Emotionale Wucht, Ueberraschung, Konflikt, Sensation (1-10)
- Massentauglichkeit: Interessiert das Millionen oder nur Nischen-Fans? (1-10)
- Aktualitaet: Breaking News oder alter Hut? (1-10)

ARTIKEL:
{articles_block}

Antworte NUR mit JSON-Array:
[{{"idx": 0, "prominenz": 7, "dramatik": 8, "masse": 9, "aktualitaet": 8, "score": 8.0, "grund": "1 Satz"}}]

Score = gewichteter Schnitt (Masse 40%, Dramatik 25%, Prominenz 20%, Aktualitaet 15%).
Nischen-Sport (Radsport-Klassiker, Kreisliga, Kunstturnen) = max Score 3.
Regionale Kleinnachrichten = max Score 2."""
    ...
    try:
        ...
        _relevance_stats["agent1_calls"] += 1
        text = resp.choices[0].message.content or ""
        start = text.find("[")
        end = text.rfind("]") + 1
        if start >= 0 and end > start:
        ...
        _relevance_stats["last_error"] = f"Agent1: {e}"
        log.warning(f"[Relevanz] Agent1 Fehler: {e}")
    return []


def _relevance_agent2_bild_dna(titles):
    """Agent 2: BILD-DNA-Experte — Passt der Artikel zum BILD-Profil?"""
    global _relevance_stats
    api_key = os.environ.get("OPENAI_API_KEY", "")
    if not api_key or not titles:
        return []
    ...
    prompt = f"""Du bist BILD-DNA-Experte. Du kennst das Profil von BILD.de perfekt. Bewerte: Wuerde BILD diesen Artikel bringen?

BILD-KERNTHEMEN (Score 7-10):
- Crime & Blaulicht: Mord, Messer, Razzia, Festnahme, Verbrechen
- Promi & Unterhaltung: Reality-TV, Royals, Showbiz, Skandale
- Grosser Sport: Bundesliga, Champions League, Nationalmannschaft, Formel 1, grosse Transfers
- Grosse Politik: Kanzler, Trump, Krieg, Terror, Wahlen
- Emotion & Human Interest: Schicksale, Kinder, Tiere, Wunder, Rekorde
- Verbraucher: Rente, Steuern, Preise, Gesundheit, Wetter

NICHT-BILD-THEMEN (Score 1-3):
- Nischen-Sport: Radsport-Klassiker, Segeln, Turnen, Leichtathletik (ausser WM/Olympia)
- Regionale Kleinnachrichten ohne ueberregionale Relevanz
- Fach-/Wirtschaftspresse: Boerse, ETFs, Anleihen
- Kultur: Oper, Theater, Literaturkritik
- Wissenschaft ohne Alltags-Bezug

ARTIKEL:
{articles_block}

Antworte NUR mit JSON-Array:
[{{"idx": 0, "bild_fit": 8, "kategorie": "Sport/Crime/Politik/Promi/...", "grund": "1 Satz"}}]

bild_fit: 1-10 (1=niemals bei BILD, 10=BILD-Pflicht)."""
    ...
    try:
        ...
        _relevance_stats["agent2_calls"] += 1
        text = resp.choices[0].message.content or ""
        start = text.find("[")
        end = text.rfind("]") + 1
        if start >= 0 and end > start:
        ...
        _relevance_stats["last_error"] = f"Agent2: {e}"
        log.warning(f"[Relevanz] Agent2 Fehler: {e}")
    return []


def _relevance_agent3_jury(titles, agent1_results, agent2_results):
    """Agent 3: Chefredakteur-Jury — Finales Urteil basierend auf Agent 1+2."""
    global _relevance_stats
    api_key = os.environ.get("OPENAI_API_KEY", "")
    if not api_key or not titles:
        return []

    # Agent-Ergebnisse zu Index mappen
    a1_map = {}
    for r in agent1_results:
        try:
            a1_map[int(r.get("idx", -1))] = r
        except (TypeError, ValueError):
            pass
    a2_map = {}
    for r in agent2_results:
        try:
            a2_map[int(r.get("idx", -1))] = r
        except (TypeError, ValueError):
            pass
    ...
    for i, t in enumerate(titles):
        ...
        a2 = a2_map.get(i, {})
        articles_block += f"\n{i}. \"{t[:100]}\"\n"
        articles_block += f"   Nachrichtenwert: {a1.get('score', '?')}/10 — {a1.get('grund', '?')}\n"
        articles_block += f"   BILD-DNA: {a2.get('bild_fit', '?')}/10 ({a2.get('kategorie', '?')}) — {a2.get('grund', '?')}\n"

    prompt = f"""Du bist der Chefredakteur von BILD.de. Zwei Analysten haben jeden Konkurrenz-Artikel bewertet. Triff die finale Entscheidung: Ist das ein Thema, das BILD FEHLT?

ENTSCHEIDUNGSREGEL:
- "relevant" = BILD sollte diesen Artikel haben, es ist eine echte Luecke
- "nicht relevant" = Kein BILD-Thema, kein Handlungsbedarf
- Score 0-100: Wie dringend sollte BILD das Thema aufgreifen?

SEI STRENG: Nur wirkliche BILD-Themen durchlassen.
- Nischen-Sport (Radsport, Handball 2. Liga, Segeln) = NICHT relevant
- Regionales ohne Sensation = NICHT relevant
- Zu kleines Thema = NICHT relevant

ARTIKEL MIT ANALYSTEN-BEWERTUNGEN:
{articles_block}

Antworte NUR mit JSON-Array:
[{{"idx": 0, "relevant": true/false, "score": 75, "grund": "1 Satz warum relevant/nicht"}}]"""
    ...
    try:
        ...
        _relevance_stats["agent3_calls"] += 1
        text = resp.choices[0].message.content or ""
        start = text.find("[")
        end = text.rfind("]") + 1
        if start >= 0 and end > start:
        ...
        _relevance_stats["last_error"] = f"Agent3: {e}"
        log.warning(f"[Relevanz] Agent3 Fehler: {e}")
    return []


def _llm_relevance_check():
    """Orchestrator: Sammelt GAP-Artikel, ruft 3 Agenten auf, cached Ergebnisse."""
    global _relevance_last_cycle_ts, _relevance_stats

    now = time.time()
    if now - _relevance_last_cycle_ts < _RELEVANCE_CYCLE_INTERVAL:
        return
    _relevance_last_cycle_ts = now

    api_key = os.environ.get("OPENAI_API_KEY", "")
    if not api_key:
        return

    # Sammle GAP-Artikel: Titel aus Feed-Cache die im Coverage-Cache als NOT covered stehen
    uncached = []  # [(title, cache_key)]
    cached_hits = 0

    for feed_type in ("competitors", "sport_competitors", "international", "sport_europa", "sport_global"):
        data = _feed_cache.get(feed_type, {}).get("data", {})
        for outlet, items in data.items():
            if not isinstance(items, list):
                continue
            for item in items:
                ...
                if not title or len(title) < 10:
                    continue
                # Nur Artikel die NICHT bei BILD sind (aus Coverage-Cache)
                cov_key = _coverage_cache_key(title, item.get("l", ""))
                with _coverage_cache_lock:
                    cov = _coverage_cache.get(cov_key)
                if cov and cov.get("covered"):
                    continue  # Bei BILD → keine Relevanz-Pruefung noetig
                # Schon im Relevanz-Cache?
                rel_key = _relevance_cache_key(title)
                with _relevance_cache_lock:
                    cached = _relevance_cache.get(rel_key)
                    if cached and (now - cached["ts"]) < _RELEVANCE_CACHE_TTL:
                        cached_hits += 1
                        continue
                uncached.append((title, rel_key))

    _relevance_stats["cache_hits"] = cached_hits
    if not uncached:
        _relevance_stats["debug"] = f"all_cached ({cached_hits})"
        return

    # Deduplizieren (gleicher Titel von mehreren Outlets)
    seen_keys = set()
    deduped = []
    for title, key in uncached:
        if key not in seen_keys:
            seen_keys.add(key)
            deduped.append((title, key))
    uncached = deduped

    # Cap: max 60 pro Zyklus (3 Batches a 20)
    if len(uncached) > 60:
        uncached = uncached[:60]

    t0 = time.time()
    total = len(uncached)
    _relevance_stats["debug"] = f"processing {total} articles"

    # Batch-weise verarbeiten (20 Titel pro Batch)
    for batch_start in range(0, len(uncached), 20):
        batch = uncached[batch_start:batch_start + 20]
        batch_titles = [t for t, k in batch]

        # Agent 1: Nachrichtenwert
        a1_results = _relevance_agent1_nachrichtenwert(batch_titles)
        # Agent 2: BILD-DNA
        a2_results = _relevance_agent2_bild_dna(batch_titles)

        # Agent-Ergebnisse indexieren
        a1_map = {}
        for r in a1_results:
            try:
                a1_map[int(r.get("idx", -1))] = r
        ...
        # Schnellentscheidung: Wenn beide Agenten eindeutig einig → kein Call 3 noetig
        need_jury = []  # Indices die Agent 3 brauchen
        for i, (title, key) in enumerate(batch):
            a1 = a1_map.get(i, {})
            a2 = a2_map.get(i, {})
            try:
                score1 = float(a1.get("score", 5))
            except (TypeError, ValueError):
                score1 = 5.0
            try:
                score2 = float(a2.get("bild_fit", 5))
            except (TypeError, ValueError):
                score2 = 5.0

            if score1 <= 3 and score2 <= 3:
                # Beide sagen klar NEIN → direkt cachen
                combined = int((score1 + score2) / 2 * 10)
                with _relevance_cache_lock:
                    _relevance_cache[key] = {
                        "relevant": False,
                        "score": combined,
                        "reason": a1.get("grund", "") or a2.get("grund", ""),
                        "kategorie": a2.get("kategorie", ""),
                        "agent1": {"score": score1, "grund": a1.get("grund", "")},
                        "agent2": {"score": score2, "grund": a2.get("grund", "")},
                        "agent3": None,
                        "ts": now,
                        "title_key": title.strip().lower()[:120],
                    }
            elif score1 >= 7 and score2 >= 7:
                # Beide sagen klar JA → direkt cachen
                combined = int((score1 + score2) / 2 * 10)
                with _relevance_cache_lock:
                    _relevance_cache[key] = {
                        "relevant": True,
                ...
                # Uneindeutig → Agent 3 (Jury) entscheidet
                need_jury.append(i)

        # Agent 3: Chefredakteur-Jury fuer uneindeutige Faelle
        if need_jury:
            jury_titles = [batch_titles[i] for i in need_jury]
            # Agent1+2 Ergebnisse nur fuer die Jury-Faelle zusammenstellen
            jury_a1 = [a1_map.get(i, {"idx": ni, "score": 5, "grund": "?"}) for ni, i in enumerate(need_jury)]
            jury_a2 = [a2_map.get(i, {"idx": ni, "bild_fit": 5, "grund": "?"}) for ni, i in enumerate(need_jury)]
            # Re-index fuer Jury-Prompt
            for ni, r in enumerate(jury_a1):
                r["idx"] = ni
            for ni, r in enumerate(jury_a2):
                r["idx"] = ni

            a3_results = _relevance_agent3_jury(jury_titles, jury_a1, jury_a2)
            a3_map = {}
            for r in a3_results:
                try:
                    a3_map[int(r.get("idx", -1))] = r
                except (TypeError, ValueError):
                    pass

            for ni, orig_i in enumerate(need_jury):
                title, key = batch[orig_i]
                a1 = a1_map.get(orig_i, {})
                a2 = a2_map.get(orig_i, {})
                a3 = a3_map.get(ni, {})
                ...
                # Jury-Ergebnis
                a3_relevant = a3.get("relevant")
                if isinstance(a3_relevant, str):
                    a3_relevant = a3_relevant.lower() in ("true", "yes", "1")
                else:
                    a3_relevant = bool(a3_relevant) if a3_relevant is not None else False
                try:
                    a3_score = int(a3.get("score", int((score1 + score2) / 2 * 10)))
                except (TypeError, ValueError):
                    a3_score = int((score1 + score2) / 2 * 10)

                with _relevance_cache_lock:
                    _relevance_cache[key] = {
                        "relevant": a3_relevant,
                        "score": min(100, max(0, a3_score)),
                        "reason": a3.get("grund", ""),
                        "kategorie": a2.get("kategorie", ""),
                        "agent1": {"score": score1, "grund": a1.get("grund", "")},
                        "agent2": {"score": score2, "grund": a2.get("grund", "")},
                        "agent3": {"relevant": a3_relevant, "score": a3_score, "grund": a3.get("grund", "")},
                        "ts": now,
                        "title_key": title.strip().lower()[:120],
                    }
    ...
    # Alte Cache-Eintraege aufraeumen (> 1h)
    with _relevance_cache_lock:
        expired = [k for k, v in _relevance_cache.items() if now - v["ts"] > 3600]
        for k in expired:
            del _relevance_cache[k]

    elapsed = int((time.time() - t0) * 1000)
    _relevance_stats["last_cycle_ms"] = elapsed
    with _relevance_cache_lock:
        final_size = len(_relevance_cache)
    _relevance_stats["debug"] = f"done: {total} articles, {elapsed}ms, cache={final_size}"
    log.info(f"[Relevanz] Zyklus: {total} Artikel bewertet, {cached_hits} cached, {elapsed}ms, cache={final_size}")
...
class PushBalancerHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        elif self.path == "/api/gap-relevance":
            self._serve_gap_relevance()
    ...
    def _serve_coverage(self):
        ...
        try:
            ...
            self._error(500, f"Coverage API error: {e}")

    def _serve_gap_relevance(self):
        """GET /api/gap-relevance — LLM-Agenten-Gremium Relevanz-Ergebnisse."""
        try:
            now = time.time()
            results = {}
            with _relevance_cache_lock:
                for key, entry in _relevance_cache.items():
                    if (now - entry["ts"]) < _RELEVANCE_CACHE_TTL:
                        title_key = entry.get("title_key", key)
                        results[title_key] = {
                            ...
                            "agent1": entry.get("agent1"),
                            "agent2": entry.get("agent2"),
                            "agent3": entry.get("agent3"),
                        }
            payload = {
                "results": results,
                "stats": dict(_relevance_stats),
                "cache_size": len(results),
                "ttl": _RELEVANCE_CACHE_TTL,
            }
            self._send_gzip(json.dumps(payload, ensure_ascii=False).encode(), "application/json; charset=utf-8")
        except Exception as e:
            self._error(500, f"Gap-Relevance API error: {e}")
