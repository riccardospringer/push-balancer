...
- The 17 Teams reviewers run locally over one ephemeral snapshot. They make no model or network calls, persist no raw push history, and fail closed when Teams article/topic deduplication state is unavailable. Missing live-push history is an explicit comparison caution, not a veto.
- Real push-recommendation payloads always require an approved title jury, final CvD jury, and complete Teams duplicate state. Normal recommendations also require the independent Teams cooldown. If the optional 17-reviewer network is enabled, its approval is required as an additional veto both at selection time and immediately before the webhook POST.
...
- A direct live refresh or relay snapshot no older than five minutes makes the comparison authoritative. Stale, empty, or unavailable live history produces an unavailable comparison marker and never blocks the independent Teams recommendation.
...
- Actual push history is retained for 90 days for aggregate analytics and live comparison; it is not a Teams eligibility or duplicate gate.
