...
def _safe_push_db_load_all(min_ts: int = 0, max_days: int = 90) -> list:
    """Lädt Pushes robust; bei DB-Fehlern kontrolliert mit leerer Liste weiter."""
    try:
        return _push_db_load_all(min_ts=min_ts, max_days=max_days)
    except Exception as exc:
        log.warning("[Tagesplan] push_db_load_all fehlgeschlagen: %s", exc)
        return []
...
def _tagesplan_background_refresh(mode="redaktion"):
    ...
    mode = mode if mode in ("redaktion", "sport") else "redaktion"
    try:
...
def _ml_build_tagesplan(background=False, mode="redaktion"):
    ...
    with _tagesplan_cache_lock:
        c = _tagesplan_cache.get(mode)
        if c is None:
            c = _TP_CACHE_EMPTY()
            _tagesplan_cache[mode] = c
        age = time.time() - c["ts"]
...
def _ml_build_tagesplan_inner(now, current_hour, mode="redaktion"):
    ...
    if not push_data:
        _tp_db_pushes = _safe_push_db_load_all()
    ...
    if not stats:
        ...
        pushes = push_data if push_data else _safe_push_db_load_all()
        if not pushes:
    ...
    if not isinstance(stats, dict):
        stats = {"global_avg": 4.77, "hour_stats": {}}
