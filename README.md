...
Protected mutation endpoints require the `X-Admin-Key` header and remain unavailable when `ADMIN_API_KEY` is not configured. Downstream consumer endpoints require `Authorization: Bearer <CONSUMER_API_KEY>` or `X-Consumer-Key` and remain unavailable when `CONSUMER_API_KEY` is not configured. The scheduled Teams claim and receipt routes use only their dedicated `X-Power-Automate-Key` and remain unavailable when `POWER_AUTOMATE_API_KEY` is not configured. `/api/teams-readiness` remains behind the internal CIDR/VPN access control.
...
During a window, the flow calls `POST /api/v1/power-automate/teams/claim` with its dedicated `X-Power-Automate-Key` and a unique `requestId`. `top` is the candidate with the absolute highest fresh, technically valid `internal_score_api` score after exact live-push and Teams-article duplicate removal. Lower-scored Sport, Breaking, section-mix, OR, local quality, pacing, or quota signals can never replace it; secondary signals only break an exact API-score tie. The separate nullable `alternative` is the highest valid candidate from the opposite Sport/non-Sport class for display and is not necessarily overall rank 2. Expected no-send outcomes return HTTP 200 with `ready=false`; the flow must not post to Teams in that branch.

The Push Balancer remains responsible for the narrow technical gates, exact duplicate protection, and the durable slot claim. One claim attempt obtains one authoritative history snapshot and reuses it for selection and the final exact-duplicate comparison; it does not renew or re-fetch history a second time during that claim. A direct live snapshot is authoritative, as is a completely parsed and persisted relay snapshot no older than five minutes whose lineage is `source=live` or `source=relay` and whose age comes from the original `snapshotTs`. Missing, stale, unknown-source, incomplete, or unpersisted history fails closed. The unique slot claim makes repeated minute runs safe and prevents more than one successful recommendation per slot.

If `ready=true`, the flow posts `messageHtml` to Teams exactly once and then calls `POST /api/v1/power-automate/teams/receipt` with the original `requestId`. A successful Teams action uses `status=sent`; every failed, timed-out, or skipped Teams action uses the terminal `status=delivery_uncertain`. The Teams connector retry policy must be `None`, because Teams may already have accepted a message even when the connector does not report success.
...
Before cutover, the internal `/api/teams-readiness` proof must report top-level `ready=true`, `teamsAlertsEnabled=true`, `transportMode=power_automate_scheduled`, `backgroundSenderEnabled=false`, `powerAutomateConfigured=true`, `scoreApi.ok=true`, `pushHistory.historyAuthoritative=true`, `slots.ok=true`, `slots.plannedToday=12`, and the exact 12 labels above. Checking only the transport key and history flag is insufficient.
...
Only the minimized hand-off crosses into Microsoft 365: slot timestamps, public article title and URL, section, Sport marker, score, and rendered recommendation HTML. Raw push history, candidate lists, reviewer details, secrets, and recipient- or employee-level activity are excluded. Backend recommendation state is retained for 45 days; Power Automate run-history retention must follow the approved tenant policy and be kept as short as operationally necessary.
