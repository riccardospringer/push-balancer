...
The live cadence is based on the deterministic Berlin weekday/hour plan. Every day starts with six evenly spaced morning decisions from `06:00` through `08:59`, followed by the `12:30` lunch decision and evenly distributed evening hot-hour slots; on Mondays the evening block starts at `17:30`. The plan contains 11 to 15 binding slots and avoids the 10/11 o'clock dead zone. Exactly one fresh Push-Balancer Top-1 recommendation is assigned to each five-minute slot. Score height, section, OR, live-push volume, Teams cooldown, pacing, and local quality juries are advisory in that mandatory mode. The only section override is the requested one-third Sport mix across recommendations already sent that Berlin day: the highest valid Sport score wins only when the remaining slots would otherwise be insufficient to reach the quota. If no valid Sport candidate exists, the canonical Top 1 still sends. A persistent unique slot claim prevents multiple worker cycles or restarts from sending more than one recommendation for the same slot.

A date-scoped recovery delay can move only the configured remainder of one day's binding plan after a late operational recovery. It is inactive on every other date. Approved recommendations sent outside the application transport can be supplied as public article URLs for exact Teams deduplication, preventing the next worker slot from recommending the same story again.

The mandatory payload carries a deterministic `slotId`. Because a webhook timeout cannot prove whether Teams accepted the POST, mandatory recommendations make only one transport attempt and record an acknowledgement-ambiguous timeout as terminal instead of risking a duplicate. The Power Automate flow should additionally reject an already processed `slotId` for end-to-end idempotency.

Legacy adaptive thresholds and secondary scores remain visible as diagnostics, but they do not gate a mandatory slot. Hard blockers are limited to missing or non-numeric canonical score data, invalid article title/link/time, non-editorial promo or fiction/TV-program pages, unavailable authoritative duplicate history, an exact live/Teams article duplicate, quiet hours, a closed slot window, or a transport failure. If the absolute API Top-1 is technically blocked or already sent, the next highest valid API-ranked article is selected.

When `PUSH_BALANCER_SCORE_API_ENABLED=1`, every current article is resolved to a CMS ID and queried through the internal score-only API. The highest fresh, technically valid, exactly deduplicated API score wins strictly unless the remaining-slot calculation makes Sport mandatory; in that case the highest valid Sport API score wins. Alert Score, CvD score, forecast, response estimate, section fit, and Breaking cannot reorder candidates. HTTP 404, timeout/5xx, stale or malformed data, missing credentials, and 401/403 never fall back to a local score. The payload carries a stable non-personal `slotId`, and transport revalidates both the exact binding slot and the live duplicate state immediately before delivery.
...
When explicitly approved and enabled, a versioned local review network fans the same ephemeral, minimized signal snapshot out to 17 deterministic specialists. It remains diagnostic in mandatory Top-1 slots: only technical validity and exact duplicate protection can stop delivery there. Missing Teams duplicate state or unavailable/stale live-push history still fails closed. The review performs no network or model call and is measured against a 50 ms local budget. It is disabled by default on Render, which is not a Teams sender.
...
After title selection, a final local CvD jury still explains article strength, OR evidence, slot fit, title quality, and the winner's margin. Its verdict cannot cancel a mandatory Top-1 slot; if title generation fails, the original article headline is used and the failed jury verdict remains visible as advisory evidence. Recommendations use the half-open five-minute interval beginning at the exact binding time (`slot <= now < slot+5m`).

Once per day, the worker sends a compact schedule containing the binding decisions, clearly marked golden-hour pairs and recovery cells, the strongest section for each matrix hour, sport context, and intentionally deprioritized hours. This planning message does not consume the recommendation cooldown. Sport is classified into separate states: prematch/starting-time notices wait, material live events must be at most 10 minutes old, finals at most 60 minutes old, and confirmed transfers/personnel decisions at most 180 minutes old. Fresh material states can receive priority only inside an open binding slot; they never bypass factual, duplicate, publication-time, cooldown, or raster gates. This sport check is local and does not introduce another external API.
...
The payload also includes structured fields such as `slotId`, `articleTitle`, `articleUrl`, `pushScore`, `pushScoreSource`, `pushScoreScoredAt`, `predictedORLabel`, `expectedOpens`, `livePushComparison`, `whyNow`, `whyPushworthy`, and the minimized review verdicts. Raw live history and full jury scorecards are never sent to Teams. `PUSH_TEAMS_CANDIDATE_LIMIT` controls the freshly ranked field. In internal-score mode, the highest fresh technically valid API score after exact duplicate removal is the final recommendation; response potential, section, sport balance, and local models can only annotate the result or break an exact score tie.
