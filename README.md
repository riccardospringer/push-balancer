...
The Push Balancer remains responsible for the narrow technical gates and the durable slot claim. One claim attempt uses at most one push-history snapshot for selection and the final exact-duplicate comparison; it does not renew or re-fetch history a second time during that claim. A direct live snapshot is authoritative, as is a completely parsed and persisted relay snapshot no older than five minutes whose lineage is `source=live` or `source=relay` and whose age comes from the original `snapshotTs`. The scheduled Power Automate path remains cloud-operational when that optional AS-network history is unavailable: it uses persisted history as a best-effort filter, while the durable article, slot, request, and receipt claims prevent repeated Teams recommendations. Set `POWER_AUTOMATE_REQUIRE_LIVE_PUSH_HISTORY=true` only when fail-closed live-push dedup is preferred over Mac-independent delivery.
