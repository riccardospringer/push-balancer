...
Election nights and comparable major events are detected automatically from the candidate field ([`app/scoring/events.py`](app/scoring/events.py)): a topic group counts as a running event only when enough candidates share it, several of them are very fresh, and at least one carries a live ticker or breaking flag. That combination distinguishes a surge from routine coverage of a long-running subject. While an event is active, its articles are exempt from the politics mix caps, the topic-saturation penalty and the politics malus, and gain `PUSH_BALANCER_EVENT_MODE_BONUS` points; running live tickers on the event stay eligible as candidates. `PUSH_BALANCER_EVENT_MODE=on|off` overrides the detection, and `/api/articles` reports the detected groups as `activeEvents`. Breaking news is detected via the "Breaking News" taxonomy node and enters immediately; all other articles enter 3 minutes after publication so the LLM check can complete first. BILD-Fit, the Germany-relevance adjustment, video special-casing, the corporate-PR malus, and the Reuters overload malus were removed — the LLM reader score covers those signals itself. Politics remains eligible for top ranks when there is a concrete current development, but stale, abstract, complex, or debate-only politics receives explicit penalties.
...
The CMS-ID source route remains backward compatible: without a query parameter it returns exactly `score` and `capturedAt`. The fixed opt-in query `?includeBreakdown=1` additionally returns `scoreBreakdown` and `orFactor` when the capture contains that complete pair; legacy captures keep the exact two-field response. New captures contain only numeric fields already displayed or applied by the candidate view. Engagement fields are `relevance`, `urgency`, `curiosity`, `freshness`, `timing`, `titleBoost`, `breaking`, `research`, `pushHistory`, and `topicSaturation`. Sport captures use `sportRelevance`, `timing`, `drama`, and `freshness`. When no browser capture exists and the server-side candidate score answers instead, `includeEditorialBreakdown=1` adds the pair as `kind: "editorial"` (without that parameter the response keeps the captured shapes only, so consumers validating them strictly are never broken by the newer form): every weighted component with its raw value and its points, including the LLM reader score share as `bildReiz`/`bildReizPoints` (40 % weight, raw LLM value in `readerScore`, the model's own short reason in `readerScoreReasoning` as a single bounded line), plus `otherAdjustments`, `baseScore`, and `freshnessMultiplier`. Its `*Points` plus `otherAdjustments` equal `baseScore`, and `baseScore` times `freshnessMultiplier` equals the delivered score. `score` remains the displayed total, while `orFactor` is the separate sorting factor. The source neither recalculates nor changes the existing score.
